...
function generateLicenseKey() { return `SHAKIL-${randomBytes(8).toString('hex').toUpperCase()}`; }
async function sendMessage(botToken, chatId, text, replyMarkup = null) {
  const url = `https://api.telegram.org/bot${botToken}/sendMessage`;
  await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ chat_id: chatId, text, reply_markup: replyMarkup ? JSON.stringify(replyMarkup) : {}, parse_mode: 'Markdown' }) });
}
  ...
  try {
    if (body.message && body.message.text === '/start') {
      ...
      if (action === 'approve') {
        ...
        await sendMessage(BOT_TOKEN, userId, `✅ *Approved!* Your License Key:\n\`${newKey}\`\n\nCopy this key and paste it into the extension.`);
