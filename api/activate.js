import clientPromise from '../lib/mongodb.js'; // .js extension added
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (license.deviceId) return res.status(200).json({ s: false, m: 'License already used.' });

    await licenses.updateOne({ _id: license._id }, { $set: { deviceId: deviceId } });
    return res.status(200).json({ s: true, m: 'Activation successful!' });
  } catch (e) {
    return res.status(500).json({ s: false, m: 'Server error during activation.' });
  }
}
